...
// ── Category icon map — keyed by name from API ─────────────────
...
const disc = (p: number, c: number) =>
  c && c > p ? Math.round((1 - p / c) * 100) : 0;
...
const CategoryPill: React.FC<{
  ...
  <button
    className={`ud__pill ${active ? 'ud__pill--active' : ''}`}
    onClick={onClick}
    title={name}
  >
    <span className="ud__pill-icon">
      {CAT_ICON[name] ?? CAT_ICON.default}
    </span>
...
const UserDashboard: React.FC = () => {
  ...
  // ── on mount: load categories + products ────────────────────
  useEffect(() => {
    dispatch(loadCategoriesThunk());
    dispatch(loadProductsThunk({
      q: '', categoryId: '', brand: '',
      sort: 'newest', page: 0, size: 12,
    }));
  }, [dispatch]);

  // ── only top-level categories (parentId === null) ────────────
  const topCategories = categories.filter(
    (c: any) => c.parentId === null || c.parentId === undefined
  );
      ...
      <nav className="ud__nav">
        <div className="ud__nav-inner">
          ...
          <div className="ud__nav-right">
            ...
            <button className="ud__logout-btn" onClick={handleLogout}>
              Sign Out
            </button>
          </div>
        </div>

        {/* ── CATEGORY PILLS ROW (below navbar) ─────────────── */}
      ...
      <div className="ud__body">
        ...
        <aside className="ud__sidebar">
          ...
          <div className="ud__sidebar-section">
            ...
            <button
              className={`ud__cat-btn ${!filters.categoryId ? 'ud__cat-btn--active' : ''}`}
              onClick={handleClearFilters}
            >
            ...
            {topCategories.map((cat: any) => (
              <button
                ...
                onClick={() => handleCategoryClick(cat.id)}
              >
                <span>{CAT_ICON[cat.name] ?? CAT_ICON.default}</span>
                {cat.name}
              </button>
            ))}
          </div>
      ...
      <div className="ud__card-body">
          ...
          <div className="ud__card-stars">
            {Array(5).fill(0).map((_, i) => (
              <span
                key={i}
                style={{
                  color: i < Math.round(product.averageRating)
                    ? '#C9A84C' : '#E8E2D4',
                }}
              >★</span>
            ))}
            <span className="ud__card-reviews">
        ...
        <div className="ud__card-prices">
          ...
          {product.compareAtPrice > product.price && (
            <span className="ud__card-compare">
              {fmt(product.compareAtPrice)}
            </span>
          )}
        </div>
        <button
          className={`ud__card-btn ${added ? 'ud__card-btn--added' : ''}`}
          onClick={handleAdd}
